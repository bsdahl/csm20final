...
bool SeatingQueue::push(const PassengerData& newPassenger)
{
	...
	while (!seatQueue.empty())
		seatQueue.pop();		// ensure that the queue reflects the passenger list of the current flight
	queueCurrentPassengers();
	...
	while (!seatQueue.empty() && (seating.size() < MAX_SEATS))
	{
		...
		{																//  increment first class counter and add passenger to flight.
			...
			if (flightPtr != nullptr)
				flightPtr->bounce();									// If passenger is of higher class, increment bounced counter.
			firstC++;
			seating.push_back(seatQueue.top());
			seatQueue.pop();
		}
		...
		{																//  add passenger to flight.
			if (flightPtr != nullptr)
			if (topSeat <= BUSI_CLASS && flightPtr->getBounced() >= MAX_BOUNCED)		// If passenger is from a higher class, and the maximum number of passengers has been
